...
export default function Agenda() {
  ...
  // FIX: filtro simples (string vazia = todos)
  ...
  const proximoHoje       = agendamentosHoje.find(a => a.hora >= minParaHora(new Date().getHours()*60 + new Date().getMinutes()));

  // FIX: clinicaId via localStorage primeiro, fallback API
  useEffect(() => {
    const init = async () => {
  ...
  const renderWeekView = () => (
    <div style={s.calWrap}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:`64px repeat(6,1fr)`, borderBottom:'1.5px solid #EFEFEF', position:'sticky', top:0, background:'#fff', zIndex:10 }}>
        ...
        {days.map(d => (
          <div key={d.date}
            style={{ padding:'12px 8px', textAlign:'center', borderRight:'1px solid #F0F0F0', background: isDiaBloqueado(d.date)?'#FFF8F8':'transparent', cursor:'pointer' }}
            onClick={() => { setDiaSelecionado(d.date); setView('Dia'); }}
          >
            <div style={{ fontSize:10, color:'#BBB', textTransform:'uppercase', letterSpacing:'0.8px', fontWeight:500 }}>{d.name}</div>
            <div style={d.today ? s.diaHoje : s.diaNum}>{d.num}</div>
            {isDiaBloqueado(d.date) && <div style={{ fontSize:9, color:'#E74C3C', fontWeight:700, marginTop:2 }}>FECHADO</div>}
          </div>
        ))}
      </div>

      {/* Corpo */}
      <div style={{ display:'flex', overflowY:'auto', maxHeight:540 }}>
        ...
        {days.map((d) => {
          ...
          return (
            <div key={d.date}
              style={{ flex:1, position:'relative', borderRight:'1px solid #F0F0F0', height:TOTAL_HEIGHT, minWidth:0 }}
              onClick={(e) => {
            ...
            >
              ...
              {apptsPosicionados.length === 0 && !isDiaBloqueado(d.date) && d.today && (
                <div style={{ position:'absolute', top:80, left:4, right:4, textAlign:'center', fontSize:10, color:'#DDD', userSelect:'none', pointerEvents:'none' }}>
                  Nenhum agendamento
                </div>
              )}
              ...
              {apptsPosicionados.map(a => {
                ...
                return (
                  <div key={a.id}
                    style={{ position:'absolute', top:t, left:esquerda, width:largura, height: 60*PX_POR_MIN-3, background:cor.bg, color:cor.text, border:`1.5px solid ${cor.border}`, borderRadius:6, padding:'5px 7px 5px 10px', cursor:'pointer', overflow:'hidden', zIndex:3, boxSizing:'border-box', borderLeft: isPendente ? `3px dashed ${cor.border}` : `3px solid ${cor.text}` }}
                    onClick={e => { e.stopPropagation(); setSelected(a); }}
                  >
                    <div style={{ fontSize:11, fontWeight:600, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                      {a.pacienteNome}{a.origem==='whatsapp'?' 📱':''}
                    </div>
                    <div style={{ fontSize:10, opacity:0.8, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                      {a.hora} · {a.dentistaNome}
                    </div>
      ...
      <div style={s.calWrap}>
        <div style={{ display:'grid', gridTemplateColumns:`64px repeat(${dentistasParaExibir.length||1},1fr)`, borderBottom:'1.5px solid #EFEFEF', position:'sticky', top:0, background:'#fff', zIndex:10 }}>
          <div style={{ padding:14, borderRight:'1px solid #F0F0F0', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <span style={{ fontSize:10, color:'#BBB', textTransform:'uppercase' }}>Hora</span>
          </div>
          {dentistasParaExibir.length > 0 ? dentistasParaExibir.map((d,i) => {
            ...
            return (
              <div key={d.id} style={{ padding:'12px 8px', textAlign:'center', borderRight:'1px solid #F0F0F0', background:`${cor.bg}44` }}>
                ...
                <button
                  style={{ marginTop:6, padding:'3px 10px', fontSize:10, background:cor.bg, color:cor.text, border:`1px solid ${cor.border}`, borderRadius:12, cursor:'pointer', fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}
        ...
        <div style={{ display:'flex', overflowY:'auto', maxHeight:560 }}>
          ...
          {dentistasParaExibir.map((dentista, di) => {
            ...
            return (
              <div key={dentista.id}
                style={{ flex:1, position:'relative', borderRight:'1px solid #F0F0F0', height:TOTAL_HEIGHT, minWidth:100, background: isDiaBloqueado(diaSelecionado)?'repeating-linear-gradient(45deg,transparent,transparent 8px,rgba(231,76,60,0.03) 8px,rgba(231,76,60,0.03) 16px)':'transparent' }}
                onClick={(e) => {
              ...
              >
                ...
                {apptsD.map(a => {
                  ...
                  return (
                    <div key={a.id}
                      style={{ position:'absolute', left:3, right:3, top:t, height:60*PX_POR_MIN-4, background:cor.bg, color:cor.text, border:`1.5px solid ${cor.border}`, borderLeft: isPendente ? `3px dashed ${cor.border}` : `3px solid ${cor.text}`, borderRadius:8, padding:'7px 9px', cursor:'pointer', overflow:'hidden', zIndex:3, boxSizing:'border-box' }}
                      onClick={e => { e.stopPropagation(); setSelected(a); }}
                    >
                ...
                {diaSelecionado === hoje && (() => {
                  ...
                  return (
                    <div style={{ position:'absolute', left:0, right:0, top:t, height:2, background:'#E74C3C', zIndex:4, pointerEvents:'none' }}>
  ...
  return (
    <div style={s.main}>
      ...
      <div style={s.kpiStrip}>
        <div style={s.kpiItem}>
          <div style={{ ...s.kpiIconSm, background:'#F0FAF6' }}>
            <Calendar size={14} color="#6BBF9E" />
          </div>
          <div>
            <div style={s.kpiNumSm}>{agendamentosHoje.length}</div>
            <div style={s.kpiLabelSm}>Consultas hoje</div>
          </div>
        </div>
        <div style={s.kpiDivider} />
        <div style={s.kpiItem}>
          <div style={{ ...s.kpiIconSm, background:'#E8F5E9' }}>
            <CheckCircle2 size={14} color="#27AE60" />
          </div>
          <div>
            <div style={{ ...s.kpiNumSm, color:'#27AE60' }}>{confirmadosHoje}</div>
            <div style={s.kpiLabelSm}>Confirmadas</div>
          </div>
        </div>
        <div style={s.kpiDivider} />
        <div style={s.kpiItem}>
          <div style={{ ...s.kpiIconSm, background:'#FFF9E6' }}>
            <AlertCircle size={14} color="#F39C12" />
          </div>
          <div>
            <div style={{ ...s.kpiNumSm, color:'#F39C12' }}>{pendentesHoje}</div>
            <div style={s.kpiLabelSm}>Pendentes</div>
          </div>
        </div>
        <div style={s.kpiDivider} />
        <div style={s.kpiItem}>
          <div style={{ ...s.kpiIconSm, background:'#EEF4FF' }}>
            <Clock size={14} color="#5B8DEF" />
          </div>
          <div>
            <div style={{ ...s.kpiNumSm, color:'#5B8DEF', fontSize:12 }}>
              {proximoHoje ? proximoHoje.hora?.slice(0,5) : '—'}
            </div>
            <div style={s.kpiLabelSm}>Próxima consulta</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
        ...
        {dentistasClinica.length > 0 && (
          <select
            ...
            style={{ padding:'7px 12px', borderRadius:8, fontSize:12, border:'1.5px solid #E8E8E8', background:'#fff', color:'#1A1A1A', cursor:'pointer', outline:'none', marginLeft:8 }}
          >
      ...
      <div style={{ display:'flex', gap:16 }}>
        ...
        <div style={{ width:210, flexShrink:0, display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <Card style={{ padding:16 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:1, textAlign:'center' }}>
              {['D','S','T','Q','Q','S','S'].map((d,i) => (
                <div key={i} style={{ fontSize:9, color:'#CCC', padding:'2px 0', fontWeight:500 }}>{d}</div>
              ))}
              {miniCells.map((d,i) => (
                <div key={i}
                  style={{ fontSize:11, padding:'4px 2px', borderRadius:'50%', cursor:d.empty?'default':'pointer', color:d.empty?'transparent':'#555', position:'relative', background:d.today?'#1A1A1A':'transparent', ...(d.today?{color:'#fff'}:{}) }}
          ...
          {/* Consultas de hoje */}
          <Card style={{ padding:16 }}>
            <div style={{ fontSize:12, fontWeight:600, color:'#1A1A1A', marginBottom:12 }}>Hoje</div>
            {agendamentosHoje.length > 0 ? (
              agendamentosHoje.slice(0,5).map(a => {
                ...
                return (
                  <div key={a.id} style={{ padding:'8px 0', borderBottom:'1px solid #F5F5F5', cursor:'pointer' }} onClick={() => setSelected(a)}>
                    <div style={{ fontSize:12, fontWeight:500, color:'#1A1A1A' }}>{a.pacienteNome}</div>
              ...
              <div style={{ fontSize:12, color:'#CCC', textAlign:'center', padding:'12px 0' }}>
                Nenhum agendamento
              </div>
            )}
          </Card>
              ...
              {dentistasClinica.map((d,i) => {
                ...
                return (
                  <div key={d.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0', borderBottom:'1px solid #F8F8F8' }}>
                    ...
                    {count > 0 && <div style={{ fontSize:10, color:cor.text, fontWeight:700 }}>{count}</div>}
                  </div>
                );
              })}
            </Card>
      ...
      {selected && (
        <div style={s.overlay} onClick={() => setSelected(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            <div style={{ ...s.modalHeader, background:corDentista(selected.dentistaNome).bg, color:corDentista(selected.dentistaNome).text }}>
              <div>
                ...
                <div style={{ fontSize:12, opacity:0.8, marginTop:3 }}>{selected.hora?.slice(0,5)} · {selected.procedimento}</div>
              </div>
      ...
      {novoModal && (
        <div style={s.overlay} onClick={() => setNovoModal(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.modalBody}>
              {feedback && (
                <div style={{ padding:'10px 14px', marginBottom:12, borderRadius:8, fontSize:13, fontWeight:500, background:feedback.tipo==='sucesso'?'#E8F5E9':'#FFEBEE', color:feedback.tipo==='sucesso'?'#27AE60':'#C62828', border:`1.5px solid ${feedback.tipo==='sucesso'?'#C8E6C9':'#FFCDD2'}` }}>
                  {feedback.tipo==='sucesso'?'✅':'❌'} {feedback.msg}
                </div>
              )}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
                ...
                <div><label style={s.label}>Email</label><input type="email" style={s.input} placeholder="email@exemplo.com" value={formData.email} onChange={e=>setFormData(p=>({...p,email:e.target.value}))} /></div>
                <div style={{ gridColumn:'1/-1' }}>
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                <button style={{ ...s.actionBtn, background:'#F0F0F0' }} onClick={() => setNovoModal(false)} disabled={carregando}>Cancelar</button>
                <button style={{ ...s.actionBtn, background:'#EEF4FF', color:'#5B8DEF', borderColor:'#C8DCF5' }} onClick={handleIncluirPaciente} disabled={carregando}>+ Incluir Paciente</button>
                <button style={{ ...s.actionBtn, background:carregando?'#CCC':'#A8D5C2', color:'#1A1A1A', cursor:carregando?'not-allowed':'pointer', borderColor:'#A8D5C2' }} onClick={handleSaveAgendamento} disabled={carregando}>
          ...
          <div style={{ ...s.modal, maxWidth:460 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.modalBody}>
              ...
              <div style={{ marginTop:20 }}>
                <button style={{ ...s.actionBtn, background:'#A8D5C2', width:'100%', borderColor:'#A8D5C2' }} onClick={() => setModalBloqueio(false)}>Fechar</button>
              </div>
            </div>
          </div>
        </div>
...
const s = {
  ...
  /* KPI Strip */
  kpiStrip: { display:'flex', alignItems:'center', gap:0, background:'#fff', borderRadius:12, border:'1.5px solid #EFEFEF', padding:'14px 24px', flexWrap:'wrap' },
  kpiItem:  { display:'flex', alignItems:'center', gap:10, flex:1, minWidth:120 },
  kpiIconSm: { width:32, height:32, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 },
  kpiNumSm: { fontSize:18, fontFamily:"'DM Serif Display',serif", color:'#1A1A1A', lineHeight:1 },
  kpiLabelSm: { fontSize:10, color:'#AAA', marginTop:2, fontWeight:400 },
  kpiDivider: { width:1, height:36, background:'#F0F0F0', margin:'0 20px', flexShrink:0 },
  ...
  viewTabActive: { background:'#fff', color:'#1A1A1A', fontWeight:600 },
